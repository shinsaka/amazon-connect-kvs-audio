...
export const getSamples = async (mediaStream) => {
    ...
    await convertStreamToRawChunks(mediaStream, rawChunks);

    return converRawChunksToRawSamples(rawChunks);
};

/**
 * Get Payload with FragmentList and convert to samples (with fragments over 1000)
 * (getMediaPayloadWithFragmentList() + getSamples())
 * @param {string} streamArn
 * @param {Array} fragments ['<flagmentNumber>', ...]
 * @param {number} chunkNumber default 1000
 * @returns object: {trackName: Array|TypedArray}
 */
export const getSamplesWithFragmentList = async (streamArn, fragments, chunkNumber = 1000) => {
    // split fragments by chunkNumber(default 1000)
    const fragmentChunks =[];
    for (let pos = 0; pos < fragments.length; pos += chunkNumber) {
        fragmentChunks.push(fragments.slice(pos, pos + chunkNumber));
    };

    const rawChunks = {};  // { trackName: [sample1, sample2, ...] }
    for (let chunk of fragmentChunks) {
        const mediaStream = await getMediaPayloadWithFragmentList(streamArn, chunk);
        await convertStreamToRawChunks(mediaStream, rawChunks);
    };
...
const convertStreamToRawChunks = async (mediaStream, rawChunks) => {
    // Append all chunks to rawChunks with convert
...
const converRawChunksToRawSamples = async (rawChunks) => {
